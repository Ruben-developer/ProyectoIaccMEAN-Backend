function respaldar(){
    ...
    const output = execSync('mongodump --db sistemadb --out ./backups/backup-`date +"%d-%m-%Y_%H:%M:%S"`', { encoding: 'utf-8' });  
...
function listar(req,res){
    const execSync = require('child_process').execSync;
    const output = execSync('ls ./backups/', { encoding: 'utf-8' });  
...
function restaurar(req, res){
    ...
    const execSync = require('child_process').execSync;    
    const output = execSync('mongorestore --db sistemadb --drop ./backups/'+backup+'/sistemadb', { encoding: 'utf-8' });  
...
function eliminar(req, res){
    ...
    const execSync = require('child_process').execSync;
    const output = execSync('sudo rm -fr ./backups/'+backup,{ encoding: 'utf-8' });  
    if(output){
        res.status(200).send({respaldo:output});
    }else{
